...
namespace Solution_Of_CF1838E {
  ...
  void solve() {
    ...
    for (i32 i = 1; i <= n; ++i) { static i32 x; x = read(); }
    i32 ans = qpow(k, m);
    i32 b = 1;
    for (i32 i = 0; i < n; ++i) {
      ans = dec(ans, mul(b, qpow(k - 1, m - i)));
      b = mul(b, m - i);
      b = mul(b, qpow(i + 1, P - 2));
    }
    fprintf(fout, "%d\n", ans);
    return void();
  }
...
signed main() { return Solution_Of_CF1838E::main(), 0; }